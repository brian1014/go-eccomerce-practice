...
func (s UserService) SignUp(input dto.UserSignup) (string, error) {
	hashPassw, err := s.Auth.CreateHashedPassword(input.Password)

	if err != nil {
		return "", err
	}

	user, errCreateUser := s.Repo.CreateUser(domain.User{
		Email:    input.Email,
		Password: hashPassw,
		Phone:    input.Phone,
	})
...
func (s UserService) Login(email string, password string) (string, error) {
	...
	if err != nil {
		return "", errors.New("user does not exists with the provided email id")
	}

	// compare password
	errVerifyPassw := s.Auth.VerifyPassword(password, user.Password)

	if errVerifyPassw != nil {
		return "", errVerifyPassw
	}
	// generate token
...
func (s UserService) GetVerificationCode(e domain.User) error {
	...
	// update user with latest verification code
	user := domain.User{
		Expirity: time.Now().Add(30 * time.Minute),
		Code:     code,
	}
...
func (s UserService) VerifyCode(id uint, code string) error {
	...
	if !time.Now().Before(user.Expirity) {
	...
	if err != nil {
		return errors.New("unable to verified user")
	}
...
func (s UserService) BecomeSeller(id uint, input dto.SellerInput) (string, error) {
	...
	if user.UserType == domain.SELLER {
		return "", errors.New("your have already joined seller program")
	}
	...
	err = s.Repo.CreateBankAccount(domain.BackAccount{
	...
	if cart.ID > 0 {
		...
		if input.Qty < 1 {
			...
			if err != nil {
				...
				return nil, errors.New("errro on deleting cart item")
			}
		} else {
			...
			if err != nil {
				...
				return nil, errors.New("errro on updating cart item")
			}
		}
	} else {
...
func (s UserService) CreateOrder(uId uint, orderRef string, pId string, amount float64) error {
	...
	if err != nil {
		log.Printf("Deleting cart items erro %v", err)
		return err
	}
